...
/// RUST_LOG=debug domain-status scan urls.txt
///
/// # Override with CLI args (takes precedence)
/// RUST_LOG=debug domain-status scan urls.txt --log-level info
///
/// # Per-module filtering via RUST_LOG
/// RUST_LOG=domain_status=debug,reqwest=info domain-status scan urls.txt
/// ```
