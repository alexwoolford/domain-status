...
mod tests {
    ...
    fn test_extract_analytics_ids_gtm_data_layer_format() {
        ...
        let html = r#"
            <script>
                ...
                })(window, document, 'script', 'dataLayer','GTM-TEST001');
            </script>
        "#;
        ...
        let gtm_ids: Vec<&str> = ids.iter()
            ...
            .map(|id| id.id.as_str())
            .collect();
        assert!(gtm_ids.contains(&"GTM-TEST001"), 
                "Should find GTM-TEST001: {:?}", gtm_ids);
    }
    ...
    fn test_extract_analytics_ids_gtm_json_format() {
        ...
        let gtm_ids: Vec<&str> = ids.iter()
            ...
            .map(|id| id.id.as_str())
            .collect();
        assert!(gtm_ids.contains(&"GTM-TEST002"), 
                "Should find GTM-TEST002: {:?}", gtm_ids);
    }
